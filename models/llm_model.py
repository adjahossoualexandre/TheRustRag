...
class TransformerLLM:
    ...
    def call(
        ...
        print(input_text)
        print('o'*24)
        parsed_response = self.parse_chat_completion(input_text=input_text, response=response)
        return parsed_response
...
class CustomLlmModelClient(ModelClient):
    ...
    def __init__(self, llm_model: TransformerLLM, auto_model: Optional[type] = AutoModel, auto_tokenizer: Optional[type] = AutoTokenizer) -> None:
        super().__init__()
        self.transformer_llm = llm_model
        self.llm_client = self.init_llm_client(auto_model=auto_model, auto_tokenizer=auto_tokenizer)

    def init_llm_client(self, auto_model: Optional[type] = AutoModel, auto_tokenizer: Optional[type] = AutoTokenizer):
        model_name = self.transformer_llm.model_name
        self.transformer_llm.init_model(model_name, auto_model=auto_model, auto_tokenizer=auto_tokenizer)
    ...
    def call(
            self,
            api_kwargs: Dict = {},
            model_type = None # For compatibility with Generator
        ):
